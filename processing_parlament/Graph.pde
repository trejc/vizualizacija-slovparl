...
class Node {
  ...
  boolean drawable;
  HashMap<String, String> datum_prikaza;
  float text_y;
  ...
  public Node(float x, float y, PStranka s) {
    ...
    this.data = s;
    text_y = 0;
    datum_prikaza = new HashMap<String, String>();
    ...
    if(s != null) {
      ...
      if(grupeBesed != null && grupeBesed.size() > 0){
        for(GrupaBesed gB: grupeBesed){
          if(!datum_prikaza.containsKey(gB.labelGrupe) || gB.besede.containsKey(datum)) {
            datum_prikaza.put(gB.labelGrupe, datum);
          }
          vseBesede+=gB.vsePojavitve(datum_prikaza.get(gB.labelGrupe));
        }
        ...
        for(GrupaBesed gB: grupeBesed){
          besede = gB.besede.get(datum_prikaza.get(gB.labelGrupe));
          ...
          if(besede != null) {
            for(String beseda : besede.keySet()) {
              ...
              //println("stranka:" + this.data.imeStranke("init", datum) + " gb:" + gB.labelGrupe + " %:" + percentage + " v:" + vseBesede + " st:" + besede.get(beseda));
              //println(start);
